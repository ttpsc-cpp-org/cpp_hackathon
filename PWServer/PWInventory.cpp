...
void PWInventory::HandleExpiredItem(InventoryItem& item)
{
    if (item.name != "Polka Dot Begonia" && item.name != "Gardening Workshop" && item.value > 0 && item.name != "White Monstera")
    {
        item.value--;
    }
    ...
    else if (item.name == "Polka Dot Begonia" && item.value < 50)
    {
        item.value++;
    }
}
